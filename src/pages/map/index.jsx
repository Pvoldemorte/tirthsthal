...
const stateOptions    = ["All States", "Madhya Pradesh", "Maharashtra", "Rajasthan", "Uttar Pradesh", "Gujarat"];
const districtOptions = ["All Districts", "Ujjain", "Indore", "Bhopal", "Varanasi", "Somnath"];
...
export default function MapPage() {
  ...
  // ── Fetch all temples from backend ──
  useEffect(() => {
    const loadTemples = async () => {
      try {
        const res  = await fetch(`${API}/temples?limit=500`);
        const data = await res.json();
        // Only keep temples that have coordinates saved
        const withCoords = (data.temples || []).filter(
          (t) => t.coordinates?.lat && t.coordinates?.lng
        );
        setAllTemples(withCoords);
      } catch (err) {
        console.error("Failed to load temples for map:", err);
      } finally {
        setLoading(false);
      }
    };
  ...
  const filtered = allTemples.filter((t) => {
    ...
    const ms  = !q || t.name?.toLowerCase().includes(q) || t.deity?.toLowerCase().includes(q);
    const mst = selState    === "All States"    || t.state    === selState;
  ...
  const handleSelectTemple = (temple) => {
    ...
    if (temple.coordinates?.lat && temple.coordinates?.lng) {
    ...
    <div className="map-filter__dropdown" onClick={() => setOpen(!open)}>
      ...
      {open && (
        <ul className="map-filter__list">
          {options.map((o) => (
            <li key={o} className={value === o ? "selected" : ""}
    ...
    <div className="map-page">
      ...
      <div className="map-page__header">
        <div className="map-page__header-left">
          ...
          <p className="map-page__sub">
            Explore {allTemples.length} temples on the map. Use filters to find temples
            by state, category or temple type.
          </p>
        </div>
        <div className="map-page__header-img">
          <img src="/images/hero-temples.jpeg" alt="temples"
            onError={(e) => e.target.style.display = "none"} />
        </div>
      </div>

      <div className="map-page__mobile-bar">
        <button className="map-page__filter-btn" onClick={() => setFilterOpen(true)}>
      ...
      <div className="map-page__body">
        ...
        <aside className="map-filter map-filter--desktop">
          ...
          <div className="map-filter__section">
            ...
            <div className="map-filter__search">
              <input type="text" placeholder="Search temple name..."
          ...
          <button className="map-filter__apply" onClick={() => {}}>
            Apply Filters
          </button>
        </aside>
        ...
        <div className="map-page__map-wrap">
          <button className="map-page__list-btn" onClick={() => setShowList(!showList)}>
            <FiList size={15} />
            {showList ? "Hide List" : `View List (${filtered.length})`}
          </button>

          {loading ? (
            <div style={{ display:"flex", alignItems:"center", justifyContent:"center",
              height:"100%", fontSize:14, color:"#6b7280" }}>
              Loading temples on map...
            </div>
          ) : (
            <MapContainer
            ...
            >
              ...
              {filtered.map((temple) => {
                // ── KEY FIX: use _id, check coordinates properly ──
                if (!temple.coordinates?.lat || !temple.coordinates?.lng) return null;
                  ...
                  >
                    <Popup>
                      <div className="map-popup">
                        ...
                        <span style={{ color: temple.deityColor || "#f4a261" }}>{temple.deity}</span>
                      </div>
                    </Popup>
                  </Marker>
                );
          ...
          <AnimatePresence>
            {showList && (
              <motion.div className="map-page__list-overlay"
                initial={{ x: "100%" }} animate={{ x: 0 }}
                exit={{ x: "100%" }} transition={{ duration: 0.3 }}>
                <div className="map-page__list-header">
                  <h4>Temples ({filtered.length})</h4>
                ...
                <div className="map-page__list-items">
                  {filtered.length === 0 ? (
                    <p style={{ padding: 16, color: "#6b7280", fontSize: 13 }}>
                      No temples with coordinates found.
                    </p>
                  ) : filtered.map((temple) => (
                    <div key={temple._id}
                      className={`map-page__list-item ${selectedTemple?._id === temple._id ? "active" : ""}`}
                      onClick={() => { handleSelectTemple(temple); setShowList(false); }}>
                      <div className="list-item__dot"
                        style={{ background: typeColors[temple.type] || typeColors.default }} />
                      <div>
        ...
        <aside className="map-sidebar">
          {selectedTemple ? (
            <div className="map-sidebar__selected">
              ...
              <span className="map-sidebar__deity-tag"
                style={{ background: `${selectedTemple.deityColor || "#f4a261"}18`,
                  color: selectedTemple.deityColor || "#f4a261" }}>
                {selectedTemple.type || selectedTemple.deity}
              </span>
              ...
              <div className="map-sidebar__temple-meta">
                ...
                <div className="meta-item">
                  ...
                  <span className="meta-item__val">
                    {selectedTemple.timings?.morning || "5:00 AM"}
                  </span>
                </div>
              </div>
              <div className="map-sidebar__btns">
                <Link to={`/temples/${selectedTemple.slug}`}
            ...
            <div style={{ padding: 20, textAlign: "center", color: "#6b7280", fontSize: 13 }}>
              <p>🛕</p>
              <p>Click a marker on the map to see temple details</p>
            </div>
          )}
              ...
              {nearbyTemples.map((t, i) => (
                <div key={i} className="map-sidebar__nearby-item">
                  ...
                  <div className="nearby-item__info">
                    ...
                    <p className="nearby-item__dist"><FiMapPin size={11} /> {t.dist}</p>
                  </div>
      ...
      <AnimatePresence>
        {filterOpen && (
          <>
            <motion.div className="map-page__filter-overlay"
              initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              onClick={() => setFilterOpen(false)} />
            <motion.div className="map-page__filter-drawer"
