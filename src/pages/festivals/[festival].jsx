...
export default function FestivalDetail() {
  ...
  const handleFav = () => {
    if (!festival) return;
    toggleFavorite({
      id: festival._id, name: festival.name,
      image: festival.image, location: festival.location,
      slug: festival.slug, type: "festival",
    });
  };

  if (loading) return (
    <div className="fest-page">
      <p style={{ padding: 60, textAlign: "center" }}>Loading festival...</p>
    </div>
  );
      ...
      <div className="fest-page__hero">
        ...
        <div className="fest-page__hero-left">
          ...
          <p className="fest-page__sub">
            {festival.location && <><FiMapPin size={14} /> {festival.location}</>}
          ...
          >
            ...
            <div style={{ display: "flex", justifyContent: "space-between",
              ...
              <button
                onClick={handleFav}
                style={{
                  background: isFavorite(festival._id) ? "#fee2e2" : "#f9fafb",
                  border: "none", borderRadius: 8, padding: "8px 14px",
                  cursor: "pointer", display: "flex", alignItems: "center", gap: 6,
                  color: isFavorite(festival._id) ? "#b91c1c" : "#374151",
                  fontWeight: 600, fontSize: 13,
                }}
              >
                <FiHeart size={15} />
                {isFavorite(festival._id) ? "Saved" : "Save"}
              </button>
            </div>

            {festival.deity       && <p><strong>Deity:</strong> {festival.deity}</p>}
            {festival.type        && <p><strong>Type:</strong> {festival.type}</p>}
            {festival.description && (
              <>
                <h3 style={{ marginTop: 20 }}>Description</h3>
                <p style={{ color: "#4b5563", lineHeight: 1.7 }}>{festival.description}</p>
              </>
            )}
            {festival.importance  && (
              <>
                <h3>Significance</h3>
                <p style={{ color: "#4b5563", lineHeight: 1.7 }}>{festival.importance}</p>
              </>
            )}
            {festival.history     && (
              <>
                <h3>History</h3>
                <p style={{ color: "#4b5563", lineHeight: 1.7 }}>{festival.history}</p>
              </>
            )}
            {festival.howToCelebrate && (
              <>
                <h3>How It's Celebrated</h3>
                <p style={{ color: "#4b5563", lineHeight: 1.7 }}>{festival.howToCelebrate}</p>
              </>
            )}
